...
- Preserved safe behavior:
  ...
  - `--source auto` falls back to REST when MCP is unavailable, times out, returns 5xx, or fails with network `TypeError`.
...
- Added MCP↔REST fixture parity assertion via Mermaid output comparison.
